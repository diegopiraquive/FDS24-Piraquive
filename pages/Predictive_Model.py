...
with tab1:
    ...
    if st.button("Predict Churn"):
        input_data = pd.DataFrame({'CreditScore_Normalized': [credit_score], 'NumOfProducts': [0], 'HasCrCard': [0], 'Balance': [balance]})
...
with tab2:
    ...
    if st.button("Predict Loan Default"):
        ...
        input_data = pd.DataFrame({'rate_of_interest': [rate_of_interest], 'loan_amount': [loan_amount], 'Upfront_charges': [upfront_charge], 'income': [0]})
